...
<?php
	...
	if(!empty($_POST)){
		...
		$count_posts_like_unlike = mysqli_query($mysqli,"select * from like_unlike_eventsposts where post_id='$post_id' AND user_id='$logged_in_user_id'");
		
		$total_like_this_post = mysqli_num_rows($count_posts_like_unlike);
		$return_array			=	array();
		if($total_like_this_post>0){
			
			$sql = "DELETE FROM like_unlike_eventsposts where post_id='$post_id' AND user_id='$logged_in_user_id'";
			$now_total_like_query = mysqli_query($mysqli,"select * from like_unlike_eventsposts where post_id='$post_id'");
			...
			$insert_like_data = "insert into like_unlike_eventsposts (user_id,post_id,type,timestamp)
